            ...
            switch (result) {
                ...
                case DialogResult.OK:
                    ...
                    if (string.IsNullOrEmpty(itemdescription)) {
                        ...
                        newBox.items.Add(new Tuple<string, bool, string?>(TotalItem, itemstatus, newBox.Name)); // currently hard coded but this adds the item to the box item list
                        lstItems.Items.Add(newBox.items.Last()); // display the newly added item in the listbox display
                    }
                    else {
                        TotalItem += " | "  + itemdescription;
                        newBox.items.Add(new Tuple<string, bool, string?>(TotalItem, itemstatus, newBox.Name));
            ...
            if (index != -1) {
                ...
                switch (result) {
                    ...
                    case DialogResult.OK:
                        ...
                        var modifiedItem = new Tuple<string, bool, string?>(TotalItem, itemstatus, newBox.Name); // set the values for the updated item
