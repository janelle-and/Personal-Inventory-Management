...
namespace Personal_Inventory_Management {
    public partial class frmMainPage : Form {
        ...
        /* function that creates the outbox panel */
        private Control CreateOutBoxControl(Box box, String boxName) {
            /* setup the panel for displaying the outbox */
            Panel panel = new Panel();
            panel.Name = box.Name;
            panel.Size = new Size(100, 150);
            panel.BackColor = Color.White;
            
            /* setup the label for the outbox panel */
            Label boxTitle = new Label();
            boxTitle.Text = boxName;
            boxTitle.Dock = DockStyle.Bottom; // set the label to show at the bottom of the panel
            boxTitle.TextAlign = ContentAlignment.MiddleCenter;
        ...
        private void Box_Click(object? sender, EventArgs e) {
            /* set the text for the search textbox and label to empty when clicking on a box */
            txtSearch.Text = "";
            lblResult.Text = "";
                ...
                if (boxPage.sending != null && boxPage.sending.items.Count > 0) {
                    foreach (var item in boxPage.sending.items) {
                        ...
                        if (existingItem != null) {
                            ...
                            OutBox.items[index] = new Tuple<string, bool, string?,int?>(existingItem.Item1, true, null,null); // change the tuple to have a true value
                        }
        ...
        private void OutBox_Click(object? sender, EventArgs e) {
            ...
            /* user clicked save on the outbox form */
            if (result == DialogResult.OK) {
                OutBox = frmOutBox.OutBox; // Update the OutBox with the new items
                /* make sure the box has items */
                if (frmOutBox.returnBox.items.Count > 0) {
                    var returnedItems = frmOutBox.returnBox.items; // save the list of items
                    /* loop through every item in the list */
                    foreach (var item in returnedItems) {
                        /* loop through each box in the flowlayoutpanel to find where the item came from */
                        foreach (var boxPanel in _boxPanelsDict) {
                            /* make sure the box name matches the box name from the list */
                            if (boxPanel.Value.Name == item.Item3) {
                                /* make sure the index of the item isnt null */
                                if(item.Item4 != null) {
                                    boxPanel.Value.items.RemoveAt(item.Item4.GetValueOrDefault()); // remove the outdated item
                                }
                                boxPanel.Value.items.Insert(item.Item4.GetValueOrDefault(),new Tuple<string, bool, string?,int?>(item.Item1, false, item.Item3,returnedItems.IndexOf(item))); // replace the item with the new outbox status
                            }
                        }
                    }
                }
        ...
        /* function that handles when the user clicks the search button */
        private void btnSearch_Click(object sender, EventArgs e) {
            ...
            bool found = false; // initialize the found variable to false since there is no found item yet
            foreach (var box in _boxPanelsDict.Values){ // loop through all the boxes
                foreach (var item in box.items){ // loop through all the items in the box
                    if (item.Item1.Contains(searchItem)){ // check if the item name contains the search text
                        found = true; // set found to true as a matching item was found
                        lblResult.Text = box.Name; // set the search result lbl text to the box the item was found in
                        return; // return from the function
                    }
                }
            }
            if (!found) {
                lblResult.Text = "Item not found"; // set the search result lbl text if no matching item was found
            }
        }
